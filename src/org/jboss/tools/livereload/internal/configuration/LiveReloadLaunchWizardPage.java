...
public class LiveReloadLaunchWizardPage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		GridDataFactory.fillDefaults()
				.align(SWT.FILL, SWT.CENTER).grab(true, false).applyTo(websocketPortText);
		final IObservableValue websocketPortTextObservable = WidgetProperties.text(SWT.Modify).observe(websocketPortText);
		...
		GridDataFactory.fillDefaults()
				.span(2, 1).align(SWT.FILL, SWT.CENTER).grab(false, false).applyTo(useProxyServerBtn);
		...
		GridDataFactory.fillDefaults()
				.align(SWT.FILL, SWT.CENTER).applyTo(proxyPortText);
		...
		GridDataFactory.fillDefaults().align(SWT.FILL, SWT.CENTER).hint(100, SWT.DEFAULT).indent(00, 10).span(2, 1).applyTo(proxyServerDescription);
		...
		// validation
		final ServerPortsValidator validationStatusProvider = new ServerPortsValidator(websocketPortTextObservable, useProxyServerSelection, proxyPortTextObservable);
