...
import org.jboss.tools.web.pagereloader.internal.service.LiveReloadWebSocketServlet.LiveReloadSocket;
...
public class LiveReloadCommandBroadcaster {
	...
	private final int proxiedPort;

	private static final ObjectMapper objectMapper = new ObjectMapper();

	public LiveReloadCommandBroadcaster(int proxiedPort, int serverPort) {
	...
	private String buildRefreshCommand(String path) throws JsonGenerationException, JsonMappingException,
		...
		command.add("refresh");
		refreshArgs.put("path", URIUtils.convert(path).toPort(proxiedPort));
	...
	public void add(LiveReloadSocket liveReloadSocket) {
		...
		Logger.debug("A new LiveReload client established a connection. Now serving {} client{}.", connections.size(),
				(connections.size() > 1 ? "s" : ""));
	}

	public void remove(LiveReloadSocket liveReloadSocket) {
		connections.remove(liveReloadSocket);
		Logger.debug("A LiveReload client closed its connection. Now serving {} client{}.", connections.size(),
				(connections.size() > 1 ? "s" : ""));
