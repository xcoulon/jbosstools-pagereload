...
public class LiveReloadScriptInjectionFilter implements Filter {
	...
	private void terminate(final HttpServletRequest httpRequest,
		...
		Logger.debug(" response type: {}", returnedContentType);
		if (isHtmlContentType(returnedContentType)) {
			Logger.debug("Inject livereload.js <script> in response for {} ({})", httpRequest.getRequestURI(),
					returnedContentType);
		...
		// to the client.
			...
			Logger.debug("*Not* injecting livereload.js <script> in response for {} ({})",
